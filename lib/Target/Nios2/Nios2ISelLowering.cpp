...
Nios2TargetLowering(Nios2TargetMachine &TM)
  : TargetLowering(TM, new Nios2TargetObjectFile()),
    ...
    HasNios264(Subtarget->hasNios264()), IsN64(Subtarget->isABI_N64()),
  ...
  setExceptionPointerRegister(Nios2::A0);
...
static bool SelectMadd(SDNode *ADDENode, SelectionDAG *CurDAG) {
  ...
  // create Nios2MAdd(u) node
  MultOpc = MultOpc == ISD::UMUL_LOHI ? Nios2ISD::MAddu : Nios2ISD::MAdd;
  ...
  SDValue CopyFromLo = CurDAG->getCopyFromReg(Chain, dl, Nios2::LO, MVT::i32,
                                              MAdd);
...
static bool SelectMsub(SDNode *SUBENode, SelectionDAG *CurDAG) {
  ...
  // create Nios2Sub(u) node
  MultOpc = MultOpc == ISD::UMUL_LOHI ? Nios2ISD::MSubu : Nios2ISD::MSub;
...
static Nios2::CondCode FPCondCCodeToFCC(ISD::CondCode CC) {
  switch (CC) {
  default: llvm_unreachable("Unknown fp condition code!");
  case ISD::SETEQ:
  case ISD::SETOEQ: return Nios2::FCOND_OEQ;
  case ISD::SETUNE: return Nios2::FCOND_UNE;
  case ISD::SETLT:
  case ISD::SETOLT: return Nios2::FCOND_OLT;
  case ISD::SETGT:
  case ISD::SETOGT: return Nios2::FCOND_OGT;
  case ISD::SETLE:
  case ISD::SETOLE: return Nios2::FCOND_OLE;
  case ISD::SETGE:
  case ISD::SETOGE: return Nios2::FCOND_OGE;
  case ISD::SETULT: return Nios2::FCOND_ULT;
  case ISD::SETULE: return Nios2::FCOND_ULE;
  case ISD::SETUGT: return Nios2::FCOND_UGT;
  case ISD::SETUGE: return Nios2::FCOND_UGE;
  case ISD::SETUO:  return Nios2::FCOND_UN;
  case ISD::SETO:   return Nios2::FCOND_OR;
  case ISD::SETNE:
  case ISD::SETONE: return Nios2::FCOND_ONE;
  case ISD::SETUEQ: return Nios2::FCOND_UEQ;
  }
}


// Returns true if condition code has to be inverted.
static bool InvertFPCondCode(Nios2::CondCode CC) {
  if (CC >= Nios2::FCOND_F && CC <= Nios2::FCOND_NGT)
    return false;

  assert((CC >= Nios2::FCOND_T && CC <= Nios2::FCOND_GT) &&
...
static SDValue CreateFPCmp(SelectionDAG &DAG, const SDValue &Op) {
  ...
  return DAG.getNode(Nios2ISD::FPCmp, dl, MVT::Glue, LHS, RHS,
...
static Nios2::FPBranchCode GetFPBranchCodeFromCond(Nios2::CondCode CC) {
  if (CC >= Nios2::FCOND_F && CC <= Nios2::FCOND_NGT)
    return Nios2::BRANCH_T;

  assert((CC >= Nios2::FCOND_T && CC <= Nios2::FCOND_GT) &&
         "Invalid CondCode.");

  return Nios2::BRANCH_F;
}
...
//SDValue Nios2TargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
//  SDValue Cond = CreateFPCmp(DAG, Op);
//
//  assert(Cond.getOpcode() == Nios2ISD::FPCmp &&
...
SDValue Nios2TargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  if (getTargetMachine().getRelocationModel() != Reloc::PIC_ && !IsN64) {
    ...
    const Nios2TargetObjectFile &TLOF =
      (const Nios2TargetObjectFile&)getObjFileLowering();

    // %gp_rel relocation
    if (TLOF.IsGlobalInSmallSection(GV, getTargetMachine())) {
      SDValue GA = DAG.getTargetGlobalAddress(GV, dl, MVT::i32, 0,
                                              Nios2II::MO_GPREL);
      SDValue GPRelNode = DAG.getNode(Nios2ISD::GPRel, dl, VTs, &GA, 1);
  ...
  unsigned GotFlag = HasNios264 ?
                     (HasGotOfst ? Nios2II::MO_GOT_PAGE : Nios2II::MO_GOT_DISP) :
                     (HasGotOfst ? Nios2II::MO_GOT : Nios2II::MO_GOT16);
  SDValue GA = DAG.getTargetGlobalAddress(GV, dl, ValTy, 0, GotFlag);
  GA = DAG.getNode(Nios2ISD::Wrapper, dl, ValTy, GetGlobalReg(DAG, ValTy), GA);
  ...
  SDValue GALo = DAG.getTargetGlobalAddress(GV, dl, ValTy, 0,
                                            HasNios264 ? Nios2II::MO_GOT_OFST :
                                                        Nios2II::MO_ABS_LO);
  SDValue Lo = DAG.getNode(Nios2ISD::Lo, dl, ValTy, GALo);
...
static unsigned getNextIntArgReg(unsigned Reg) {
  assert((Reg == Nios2::A0) || (Reg == Nios2::A2));
  return (Reg == Nios2::A0) ? Nios2::A1 : Nios2::A3;
}
...
Nios2TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
    ...
    case CCValAssign::Full:
      if (VA.isRegLoc()) {
        if ((ValVT == MVT::f32 && LocVT == MVT::i32) ||
          ...
          SDValue Lo = DAG.getNode(Nios2ISD::ExtractElementF64, dl, MVT::i32,
                                   Arg, DAG.getConstant(0, MVT::i32));
          SDValue Hi = DAG.getNode(Nios2ISD::ExtractElementF64, dl, MVT::i32,
  ...
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
    if (IsPICCall && G->getGlobal()->hasInternalLinkage()) {
      OpFlag = IsO32 ? Nios2II::MO_GOT : Nios2II::MO_GOT_PAGE;
      unsigned char LoFlag = IsO32 ? Nios2II::MO_ABS_LO : Nios2II::MO_GOT_OFST;
    ...
    if (IsN64 || (!IsO32 && IsPIC))
      OpFlag = Nios2II::MO_GOT_DISP;
    else if (!IsPIC) // !N64 && static
      OpFlag = Nios2II::MO_NO_FLAG;
    else // O32 & PIC
      OpFlag = Nios2II::MO_GOT_CALL;
  ...
  if (IsPICCall || !GlobalOrExternal) {
    // copy to T9
    unsigned T9Reg = IsN64 ? Nios2::T9_64 : Nios2::T9;
  ...
  if (IsPICCall) {
    unsigned GPReg = IsN64 ? Nios2::GP_64 : Nios2::GP;
    EVT Ty = IsN64 ? MVT::i64 : MVT::i32;
    RegsToPass.push_back(std::make_pair(GPReg, GetGlobalReg(DAG, ValTy)));
  }
...
Nios2TargetLowering::LowerFormalArguments(SDValue Chain,
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i, ++FuncArg) {
    ...
    if (Flags.isByVal()) {
      ...
      if (IsO32) {
        ...
        LastFI = CopyNios264ByValRegs(MF, Chain, dl, OutChains, DAG, VA, Flags,
                                     MFI, IsRegLoc, InVals, Nios2FI,
    ...
    if (IsRegLoc) {
      ...
      if (RegVT == MVT::i32)
        ...
        RC = &Nios2::CPU64RegsRegClass;
      else if (RegVT == MVT::f32)
        RC = &Nios2::FGR32RegClass;
      else if (RegVT == MVT::f64)
        RC = HasNios264 ? &Nios2::FGR64RegClass : &Nios2::AFGR64RegClass;
      else
      ...
      if ((RegVT == MVT::i32 && ValVT == MVT::f32) ||
        ...
        ArgValue = DAG.getNode(Nios2ISD::BuildPairF64, dl, MVT::f64,
                               ArgValue, ArgValue2);
      }
  ...
  if (isVarArg) {
    ...
    const uint16_t *ArgRegs = IsO32 ? O32IntRegs : Nios264IntRegs;
    ...
    const TargetRegisterClass *RC = IsO32 ?
      ...
      (const TargetRegisterClass*)&Nios2::CPU64RegsRegClass;
...
EVT Nios2TargetLowering::getOptimalMemOpType(uint64_t Size, unsigned DstAlign,
                                            unsigned SrcAlign, bool IsZeroVal,
                                            bool MemcpyStrSrc,
                                            MachineFunction &MF) const {
  if (Subtarget->hasNios264())
    return MVT::i64;

  return MVT::i32;
}

unsigned Nios2TargetLowering::getJumpTableEncoding() const {
  if (IsN64)
